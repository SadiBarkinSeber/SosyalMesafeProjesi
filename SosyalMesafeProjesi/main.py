...
def get_mouse_points(event, x, y, flags, param):
    # Sosyal mesafe 2 metredir proje evrensel olması için 2 metrenin karşılığı olarak 6 fit kullandım.
    # Videonun kare sıfırındaki 4 noktayı işaretlemek için kullanılır
    # Videonun sıfır karesinde 6 fit uzakta olan 2 noktayı işaretlemek için kullanılır
...
# Videonun sonuna kadar her kareyi işleyin
while cap.isOpened():
    frame_num += 1
    ret, frame = cap.read()
    ...
    if frame_num == 1:
        # # Kullanıcıdan paralel noktaları ve birbirinden 6 fit aralıkla
        # iki noktayı işaretlemesini isteyin. Order bl, br, tr, tl, p1, p2
        while True:
            image = frame
    ...
    # ROI poligonunu çizin
    pts = np.array(
        [four_points[0], four_points[1], four_points[3], four_points[2]], np.int32
    )
    cv2.polylines(frame, [pts], True, (0, 255, 255), thickness=4)

    # DNN kullanarak kişiyi ve sınırlayıcı kutuları tespit edin
